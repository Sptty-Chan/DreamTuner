...
commandArray="pm trim-caches 9999G
"
SAVEIFS=$IFS
IFS=$'\n'
...
do
  eval $comm
  echo "mengeksekusi command $comm"
done

# GET SCREEN RESOLUTION & DPI
getResolution=$(wm size)
getResolution=($getResolution)
getResolution="${getResolution[0]}"
IFS=$'x'
getResolution="${getResolution//'Physical size: '/''}"
widthXHeight=($getResolution)
getWidth="${widthXHeight[0]}"
getHeight="${widthXHeight[1]}"
getDpi=$(wm density)
IFS=$'\n'
getDpi=($getDpi)
getDpi="${getDpi[0]}"
IFS=$SAVEIFS
getDpi="${getDpi//'Physical density: '/''}"

packageList=$(pm list packages -3)
arr=($packageList)
IFS=$SAVEIFS
for i in "${arr[@]}"
do
  commandExec="${i//'package:'/'am force-stop '}"
  if [ "$commandExec" != "am force-stop com.termux" ]
  then
    eval $commandExec
    echo "Paksa berhenti aplikasi ${i//'package:'/''}"
  fi
done
...
dpi=$(($getDpi / 2))
xc="x"
echo "Merubah resolusi & dpi layar menjadi 50%"
chRaD=$(wm size $width$xc$height && wm density $dpi)

echo "Paksa berhenti aplikasi me.piebridge.brevent"
fin=$(am force-stop com.termux)
